/* The following (not including LookAt and Vertex3f) could either be
 i mplemented into* your file or they can stand as a separate file called
...
void draw_line(float x0, float y0, float x1, float y1, CommandHandle& ch)
{
  GLint viewport[4];
  int i;
  float x,y;
  ...
  for (i = 0; i <= length; i++) {
    ...
    if(xi<MAX_IMG_WIDTH && yi<MAX_IMG_HEIGHT) {
        //Draw on window
        ch.main_window[yi][xi][0] = (GLbyte) 255;
        ch.main_window[yi][xi][1] = (GLbyte) 255;
        ch.main_window[yi][xi][2] = (GLbyte) 255;
    }
